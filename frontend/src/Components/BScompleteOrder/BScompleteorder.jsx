  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch products
        const productsResponse = await axios.get(`http://localhost:3000/api/products/user/${userId}`);
        ...
        const notificationsResponse = await axios.get(`http://localhost:3000/api/consumernotif/${userId}`);
        setOrders(notificationsResponse.data);

        // Fetch canceled orders
        const canceledOrdersResponse = await axios.get(`http://localhost:3000/api/cancellednotif/${userId}`);
...
export default BScompleteorder;
