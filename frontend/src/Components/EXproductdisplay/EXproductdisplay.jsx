...
const EXproductsdisplay = () => {
  ...
  useEffect(() => {
    if (!product) {
      ...
      const fetchProduct = async () => {
        try {
          const response = await axiosInstance.get(`api/exproducts/${id}`);
  ...
  const addToCart = async () => {
    ...
    try {
      const response = await axiosInstance.post('api/exadd-to-cart', {
        consumerId: userId,
        productId: id,
        quantity: 1,
      });
...
export default EXproductsdisplay;
