...
const RSproductlist = () => {
  ...
  const [rsSidebarVisible, setRsSidebarVisible] = useState(true);
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        const response = await axiosInstance.get(`api/Rsproducts/user/${userId}`);
  ...
  const handleArchiveProduct = async (id) => {
    try {
      await axiosInstance.post(`api/rsproducts/archive/${id}`);
  ...
  const handleSaveEdit = async () => {
    try {
      ...
      const response = await axiosInstance.put(`api/rsproducts/${editingProduct._id}`, editingProduct);
