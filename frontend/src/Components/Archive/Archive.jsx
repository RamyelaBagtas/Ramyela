...
const Archive = () => {
  ...
  const [sortDirection, setSortDirection] = useState('asc'); // Added sortDirection state
  ...
  useEffect(() => {
    const fetchArchivedProducts = async () => {
      try {
        const response = await axiosInstance.get(`api/sproducts/archive/${userId}`); // Use userId in the endpoint
        const productsWithImages = response.data.map(product => ({
          ...product,
          imageUrl: arrayBufferToBase64(product.image.data),
          expirationDate: formatDate(product.expirationDate) // Format the date
        }));
  ...
  const handleNavigateToProductList = () => {
    navigate('/sproductlist'); // Navigate to the Sproductlist route
  };

  const handleUnarchiveProduct = async (productId) => {
    try {
      await axiosInstance.post(`api/sproducts/unarchive/${productId}`);
      ...
      enqueueSnackbar('Error unarchiving product', { variant: 'error' }); // Show error notification
    }
  };
